...
import { UserRegistration, UserCredentials } from '../modules/models/user.model';
...
//  baseBxUrl:string = "https://b24-qbgwgy.bitrix24.com/rest/"
  ...
  getBX(url:string, params:any = {}, fnSuccess:Function = null,fnError:Function = null,showLoading:Boolean = true) {
  ...
  async getBXAsync(url:string, params:any = {}, fnSuccess:Function = null,fnError:Function = null,showLoading:Boolean = true) {
    var httpOptions = this.getBXHttpOptions(params);
    if (showLoading)
    this.requestLoader(true);

    await this.http.get(this.baseBxUrl + url,httpOptions).toPromise().then(async data => {
      this.requestLoader(false);
      if (fnSuccess != null)
      await fnSuccess(data);
    }).catch(err => {
      this.handleBxErrors(err);
      if (fnError != null)
      fnError(err);
  ...
  postBX(url:string, params:Object = {}, fnSuccess:Function = null,fnError:Function = null,showLoading:Boolean = true) {
