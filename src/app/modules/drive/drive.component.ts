...
export class DriveComponent implements OnInit {
  ...
  constructor(
    ...
    this.addContextMenuItem("კოპირება", "file_copy", (card) => {
      ...
      if (obj != null) {
        ...
        this.commonService.setCookieExpire(
          "copyObjectType",
          obj.type,
          expireDate
        );
    ...
    this.addContextMenuItem("წაშლა", "delete", (card: HTMLElement) => {
      ...
      if (obj != null) {
        var url = "";
        if (obj.type == "folder") url = "disk.folder.deletetree";
        else url = "disk.file.delete";
    ...
    this.route.params.subscribe((params) => {
      if (params["id"] != null) this.openFolder(params["id"]);
    });
  ...
  getRootFolder() {
    var filterBtWg =
      "?filter[ENTITY_ID]=" +
      (Authuser.workgroup.active ? Authuser.workgroup.ID : Authuser.bxUserID);
    var filterType = Authuser.workgroup.active
  ...
  uploadFile(
    ...
    this.commonService.getBX("disk.folder.uploadfile", params, (res) => {
      ...
      this.httpClient.post<any>(uploadUrl, fd).subscribe(
        (data) => {
          // location.reload();
          // this.getRootFolder();
          if (reloadPage) {
  ...
  async fileSelect(file: any) {
    var filesList: FileList = file.target.files;
    if (filesList && filesList.length > 0) {
      for (var i = 0; i < filesList.length; i++) {
        var reader = new FileReader();
        var $this = this;
        var fileName = filesList[i].name + " ( " + new Date().valueOf() + ")";
        var thisFile: File = filesList[i];
        var fileContent = await new Response(file).text();
  ...
  createNewFolderClick() {
    ...
    if (
      ...
      this.newFolderName.trim().length == 0
    )
      return;
    ...
    var filter =
      ...
      "&data[NAME]=" +
      this.newFolderName;
    this.commonService.getBX("disk.folder.addsubfolder" + filter, {}, (data) =>
      location.reload()
    );
  }
  ...
  renameConfirmClick(id: number, value: string, type: string = "file") {
    this.commonService.getBX(
      "disk." + type + ".rename?id=" + id + "&newName=" + value,
      {},
      () => location.reload(),
      () => location.reload()
    );
  ...
  copyOrMoveToCurrentID() {
    ...
    if (currentFolderID > 0) {
      try {
        ...
        var typeStr =
          type == "folder" ? "folder" : type == "file" ? "file" : null;
        var actionStr =
          action == "copy" ? "copyto" : action == "move" ? "moveto" : null;
