...
export class TasksComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (Authuser.workgroup.active) this.getTaskList();
    else Authuser.workgroup.onWorkGroupChange.push(this.getTaskList.bind(this));
  ...
  getTaskList(startID: number = 0) {
    ...
    var group = {};
    var filterByWorkgroup = Authuser.workgroup.active
      ? "?order[id]=desc&filter[GROUP_ID]=" + Authuser.workgroup.ID
      : "";
  ...
  pageChanged(x) {
    if (
      ...
      x.pageIndex >= Math.ceil(x.length / x.pageSize) - 2
    )
      this.getTaskList(
  ...
  saveNewTask() {
    ...
    var groupIDFilter = Authuser.workgroup.active
      ? "&fields[GROUP_ID]=" + Authuser.workgroup.ID
      : "";
    var titleFilter = "?fields[TITLE]=" + this.newTask.title;
    var descrFilter =
      "&fields[DESCRIPTION]=" + encodeURI(this.newTask.description);
    var responsibleFilter =
      "&fields[RESPONSIBLE_ID]=" +
      (this.newTask.responsibleId == null ||
      this.newTask.responsibleId.length == 0
        ? Authuser.bxUserID
        : this.newTask.responsibleId);
    var taskID = this.newTask.id > 0 ? "&taskId=" + this.newTask.id : "";
    var deadlineFilter =
      "&fields[DEADLINE]=" +
      (this.newTask.deadline == null ? "" : this.newTask.deadline.toJSON());
    var filter =
      titleFilter +
      descrFilter +
      responsibleFilter +
    ...
    var taskActionUrl =
      this.newTask.id > 0 ? "tasks.task.update" : "tasks.task.add";

    if (
      !this.newTask.title ||
      !this.newTask.description ||
      Number(this.newTask.responsibleId) == 0 ||
  ...
  taskDescription: InputComponent;
  addNewTask() {
  ...
  showTaskDetails(row: Task) {
    ...
    this.commonService.getBX("tasks.task.get" + filter, {}, (data) => {
      ...
      if (
        this.newTask.ufTaskWebdavFiles &&
        this.newTask.ufTaskWebdavFiles.toString() != "false" &&
        this.newTask.ufTaskWebdavFiles.length > 0
      ) {
      ...
      this.taskDescription.richTextFunction = (text: string) => {
        if (this.newTask.attachedFiles.length > 0) {
          this.newTask.attachedFiles.forEach((element) => {
            ...
            text = text.replace(
              "[DISK FILE ID=" + element.id + "]",
              replaceObj
            );
            text = text.replace(
              "[DISK FILE ID=n" + element.object_id + "]",
              replaceObj
            );
          });
        }
  ...
  getTaskFiles(fileIDs: []) {
    ...
    fileIDs.forEach((x) => {
      this.commonService.getBX(
        "disk.attachedObject.get?id=" + x,
        {},
        (data) => {
  ...
  completeTask() {
    this.commonService.getBX(
      "tasks.task.complete?taskId=" + this.newTask.id,
      {},
      (data) => {
        this.reloadTasks();
      }
  ...
  getComments() {
    ...
    this.commonService.getBX(
      "task.commentitem.getlist?taskId=" + this.newTask.id,
      {},
      (data) => {
        ...
        while (indexOfRemoveItem > -1 || indexOfRemoveItem == -2) {
          indexOfRemoveItem = this.allComments.findIndex(
            (x) => x["POST_MESSAGE"] == "commentAuxTaskInfo"
          );
        ...
        this.newTask.lastComment = new Comment(
          lastComment["AUTHOR_NAME"],
          lastComment["POST_MESSAGE"],
          lastComment["POST_DATE"]
        );

        this.allComments[this.allComments.length - 1][
          "POST_MESSAGE"
        ].toString();
      }
    );
  ...
  addComment() {
    if (
      this.newTask.newComment == null ||
      this.newTask.newComment.length == 0 ||
    ...
    this.commonService.getBX(
      "task.commentitem.add?taskId=" + this.newTask.id,
      params,
      () => {
        this.showCommentsPopup = false;
  ...
  getStatusName(status: number) {
    ...
    if (filterStatus != null) return filterStatus[0]["nameGeo"];
    else return status;
  }
  canEdit() {
    if (Number(this.newTask.id) > 0 && Number(this.newTask.status) != 5)
      return true;
    else if (Number(this.newTask.id == 0) || this.newTask.id == null)
      // თუ ახალს ვაკეთებ
      return true;
  ...
  deleteTask(taskID) {
    ...
    if (confirm)
      this.commonService.getBX(
        "tasks.task.delete?taskId=" + taskID,
        {},
        (data) => {
          this.reloadTasks();
        },
